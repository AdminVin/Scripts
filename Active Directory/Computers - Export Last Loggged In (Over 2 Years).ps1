...
Get-ADComputer -Properties Enabled,DNSHostName,LastLogonDate,LastLogonTimeStamp -Filter {LastLogonTimeStamp -lt $LastLogonDate } -SearchBase $SearchOU | Select-Object Enabled,DNSHostName,LastLogonDate,DistinguishedName | Export-CSV "Computers-ExportLastLoggedIn(Over2Years).csv"
